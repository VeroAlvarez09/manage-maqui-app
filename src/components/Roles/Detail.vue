            ...
            <div class="row justify-center">
              <div class="col-12 col-md-4" v-for="(value,key) in data">
                ...
                <div v-if="key === 'permissions'">
                  ...
                  <label v-for="permission in value">[{{ permission.name }}] </label>
                </div>
              </div>
            </div>
          </q-card-section>
...
export default {
  ...
  methods: {
    edit() {
      this.$router.push('/admin/role/new/' + this.id)
    },
    detroy() {
      this.showLoading("Eliminando rol...");
      ServiceApi.detroy("/admin/roles/" + this.id)
        .then(response => {
          if (response.status) {
            this.back();
          } else {
    ...
    onRequest() {
      ...
      ServiceApi.get("/admin/roles/" + this.id).then(response => {
        const {status, message, role} = response.data;
        if (status) {
          this.data = role[0];
        } else {
