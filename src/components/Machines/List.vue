  ...
  <q-page>
    <div class="q-pa-md">
      ...
      <q-table
        ...
        loading-label="Consultando maquinas"
          ...
          <q-tr :props="props">
            <q-td>
              <q-item
                ...
                clickable
                tag="a"
                :to="'/machine/detail/' + props.row.id"
              >
                {{ props.row.id }}
              </q-item>
            </q-td>
            ...
            <q-td
              ...
              v-if="col.name !== 'id'"
            >
              {{ col.name === 'company' ? col.value.businessName : col.value }}
            </q-td>
            <q-td auto-width>
...
export default {
  ...
  methods: {
    detail(id) {
      this.$router.push("/machine/detail/" + id);
    },
    edit(id) {
      this.$router.push("/machine/new/" + id);
    },
    onRequest() {
      this.loading = true;
      ServiceApi.get(`/machine?page=${this.pagination.page},perPage=${this.pagination.rowsPerPage}`, {})
        .then(response => {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {name: "company", align: "left", label: "Compañia", field: "company"}
      ],
      data: []
    };
  }
