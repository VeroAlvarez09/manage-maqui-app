...
export default {
  ...
  methods: {
    edit() {
      this.$router.push('/admin/module/new/' + this.id)
    },
    detroy() {
      this.showLoading("Eliminando permiso...");
      ServiceApi.detroy("/admin/modules/" + this.id)
        .then(response => {
          if (response.status) {
            this.back()
          } else {
    ...
    onRequest() {
      this.showLoading("consultando modulo...");
      ServiceApi.get("/admin/modules/" + this.id).then(response => {
