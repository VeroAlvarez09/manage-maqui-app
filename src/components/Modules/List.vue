  ...
  <q-page>
    <div class="q-pa-md">
      ...
      <q-table
        ...
        loading-label="Consultando modulos"
          ...
          <q-tr :props="props">
            <q-td>
              <q-item
                ...
                clickable
                tag="a"
                :to="'/admin/module/detail/' + props.row.id"
              >
                {{ props.row.id }}
              </q-item>
            </q-td>
            ...
            <q-td
              ...
              :key="col.name"
              :props="props"
              v-if="col.name !== 'id' && col.name !== 'permissions' && col.name !== 'actions'"
            >
              {{ col.value }}
            </q-td>
...
export default {
  ...
  methods: {
    detail(id) {
      this.$router.push("/admin/module/detail/" + id);
    },
    edit(id) {
      this.$router.push("/admin/module/new/" + id);
    },
    onRequest() {
      this.loading = true;
      ServiceApi.get("/admin/modules")
        .then(response => {
          const {modules, status, message} = response.data;
          if (status) {
            this.data = modules;
          }
          this.loading = false;
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {name: "permissions", align: "left", label: "Permisos", field: "permissions"},
      ],
      data: []
    };
  }
